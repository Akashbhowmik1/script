...
const button = document.getElementById("actionButton");
  ...
  let state = localStorage.getItem("state") || "waiting"; // "waiting", "counting", "ready"

  function updateProgress() {
    progressText.innerText = `${currentStep}/${TOTAL_STEPS} Complete`;
  }
  ...
  function generateCode() {
    // Set your custom code logic here
    const code = "ABC123"; // Example
  ...
  // Restore previous state if page refreshed
  if (state === "counting" && countdown > 0) {
    startCountdown();
  } else {
    updateButton();
  }
